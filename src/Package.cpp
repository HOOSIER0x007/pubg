...
void Package::GenerateClass(const UEClass& classObj)
{
		...
		c.PredefinedMethods.push_back(IGenerator::PredefinedMethod::Inline(tfm::format(R"(	static UClass* StaticClass()
	{
		...
		if (!ptr) ptr = UObject::FindClass(%s);
		return ptr;
	...
	{
		static UClass* ptr = nullptr;
		if (!ptr) ptr = UObject::FindClass(0x%08x);
		return ptr;
	})", fnv_hash_runtime(c.FullName.c_str()))));
	}
...
std::string Package::BuildMethodSignature(const Method& m, const Class& c, bool inHeader) const
{
	...
	ss << (from(m.Parameters)
		...
		>> select([](auto&& param) { return (param.PassByReference ? "const " : "") + param.CppType + (param.PassByReference ? "& " : param.ParamType == Type::Out ? "* " : " ") + ((param.Name == "return") ? "_return" : param.Name); })
		>> concatenate(", "));
	ss << ")";
