...
public:
	...
	virtual unsigned GetGlobalMemberAlignment() const
	{
		return 0x8;
	}
	...
	virtual unsigned GetClassAlignas(const std::string& name) const
	{
	...
	using VirtualFunctionPatterns = std::vector<std::tuple<const char*, const char*, unsigned, const char*>>;
...
protected:
	std::unordered_map<std::string, unsigned> alignasClasses;
